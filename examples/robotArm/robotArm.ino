...
WidgetRobotArm robotArm = WidgetRobotArm();
...
void robotArmControlCb(void)
{
	baseControl(robotArm.getBase());
	shoulderControl(robotArm.getShoulder());
	elbowControl(robotArm.getElbow());
	wristControl(robotArm.getWrist());
	gripperControl(robotArm.getGripper());	
}
...
void setup()
{
	...
	delay(500);
	//接收机械臂控制命令
	robotArm.begin(robotArmControlCb);
}
